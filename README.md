...
hash_policy: metadata_fast_path  # always | metadata_fast_path | copy_then_hash
...
| `hash_policy` | 何時算內容 hash:`always`(每次先 hash)、`metadata_fast_path`(預設;未變檔案可跳過重 hash)、`copy_then_hash`(第一次 regular-file 同步先 copy 再 hash) |
...
`hash_policy` 決定 engine 在 ingest 前後何時取得內容 hash:

| 值 | 行為 |
|---|---|
| `always` | 每次 ingest 都先讀 source 算 hash,再決定是否要 copy |
| `metadata_fast_path`(預設) | regular file 先比 `inode + size + mtime_ns`; metadata 沒變就沿用舊 hash,不重讀整個檔案 |
| `copy_then_hash` | 第一次 regular-file ingest 先 copy 到框架 staging 區,再 hash staged copy,最後決定正式 target。這讓首次同步也能避免 pre-hash,而 hash-aware mapper 一樣可用 |
...
### `--progress` 旗標
`run` / `scan` / `reconcile` 都支援 `--progress`,會在初始 scan 階段印 `tqdm` 進度條:

```
sendout-flatten:  42%|████▏     | 647/1523 [00:12<00:16, 54.2 unit/s]
```

需要先裝 extra:`pip install fferyman[progress]`(或 `pip install -e '.[progress]'`)。沒裝也可以帶 `--progress`,會印警告然後退回無進度條模式,不會 crash。
